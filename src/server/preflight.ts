...
export function assertSlideLayout(layout: LayoutInfo, slides: SlideInfo[], blocks: Record<string, BlockInfo>) {
  ...
  for (const [index, page] of layout.pages.entries()) {
    ...
    const visit = (node: ElementInfo, inherited?: string, clips: ElementInfo[] = []) => {
      ...
      if (owner && owner !== expected) {
        ...
        throw new RenderFailure(message, [{ code: 'slide-content-moved', severity: 'error', message, page: index + 1, blockId: id }]);
      }
      ...
      if (node.kind === 'Rect' && node.style.opacity !== 0 && (outside(node, paper) || clips.some(box => outside(node, box)))) {
        ...
        throw new RenderFailure(message, [{ code: 'slide-overflow', severity: 'error', message, page: index + 1, blockId: id }]);
      }
      const nextClips = node.style?.overflow === 'Hidden' ? [...clips, node] : clips;
      node.children.forEach(child => visit(child, id, nextClips));
    };
...
type Mark = { node: ElementInfo; blockId?: string; heading?: ElementInfo; fixed: boolean };
function blockIdOf(node: ElementInfo, inherited?: string) {
...
export function inspectLayout(layout: LayoutInfo, blocks: Record<string, BlockInfo>) {
  ...
  const add = (issue: ReviewIssue) => {
    const key = `${issue.code}:${issue.page}:${issue.blockId ?? ''}`;
    if (!seen.has(key)) { seen.add(key); issues.push(issue); }
  };
  for (const [index, page] of layout.pages.entries()) {
    const number = index + 1;
    const leaves: Mark[] = [], headings: Mark[] = [];
    const paper = { x: 0, y: 0, width: page.width, height: page.height };
    const visit = (node: ElementInfo, inherited?: string, fixed = false, heading?: ElementInfo, clips: ElementInfo[] = []) => {
      ...
      if (visible && node.style.opacity !== 0) {
        ...
        if (![node.x, node.y, node.width, node.height].every(Number.isFinite) || outside(node, paper)) {
          add({ code: 'page-overflow', severity: 'error', page: number, blockId,
            message: `Content extends beyond page ${number}. Reduce the fixed width or height, or allow the content to flow onto another page.` });
        }
        if (clips.some(box => outside(node, box))) {
          add({ code: 'clipped-content', severity: 'error', page: number, blockId,
            message: `Content is clipped by a fixed container on page ${number}. Remove the clipping or give it enough space.` });
        }
      }
      const nextClips = node.style.overflow === 'Hidden' ? [...clips, node] : clips;
      for (const child of node.children) visit(child, blockId, fixed, heading, nextClips);
    };
    ...
    for (const mark of leaves) {
      if (!mark.fixed && furniture.some(other => overlaps(mark.node, other.node))) {
        add({ code: 'page-furniture-overlap', severity: 'error', page: number, blockId: mark.blockId,
          message: `Content overlaps the running header or footer on page ${number}. Reduce the unbreakable content or adjust the page margins.` });
      }
    }
